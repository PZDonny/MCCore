...
public class SlimeHandlerImpl implements SlimeHandler<SlimeWorld, SlimeLoader> {
    ...
    public WrappedSlimeLoaderImpl createFileLoader(@NotNull File file) {
        return new WrappedSlimeLoaderImpl(new FileLoader(new File("slime_worlds")));
    }
    ...
    public WrappedSlimeWorldImpl createEmptyWorld(@NotNull String worldName, boolean readOnly) {
        return new WrappedSlimeWorldImpl(AdvancedSlimePaperAPI.instance().createEmptyWorld(worldName, readOnly, new SlimePropertyMap(), null));
    }

    @Override
    public WrappedSlimeWorldImpl generateEmptyArenaWorld(@NotNull String emptyWorldName, World.@Nullable Environment environment) {
