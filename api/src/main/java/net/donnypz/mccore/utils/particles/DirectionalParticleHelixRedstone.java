...
public class DirectionalParticleHelixRedstone extends ParticleHelix{
    ...
    protected void spawnParticle(Location location) {
        ...
        location.getWorld().spawnParticle(CoreAPI.getVersionHandler().getDustParticle(), location, amount, 0,0,0, new Particle.DustOptions(color, particleSize));
    }
    ...
    protected void spawnParticle(Collection<Player> players, Location location) {
        ...
        for (Player player : players){
            ...
            player.getWorld().spawnParticle(CoreAPI.getVersionHandler().getDustParticle(), location, amount, 0,0,0, new Particle.DustOptions(color, particleSize));
        }
    }
}
