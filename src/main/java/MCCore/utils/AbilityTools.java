...
public class AbilityTools {
    ...
    public static List<Player> getNearbyPlayers(Player p, double d, Map<UUID, Long> cooldown, boolean sendMSG, String prefix){
        Collection<Entity> coll = p.getNearbyEntities(d, d, d);
        if (p.getNearbyEntities(d,d,d).stream().noneMatch(en->en instanceof Player)) {
            if (cooldown != null) cooldown.remove(p.getUniqueId());
            if (sendMSG){
    ...
    public static List<Player> getNearbyPlayers(Player p, double d, Map<UUID, Long> cooldown, Location l, boolean sendMSG, String prefix){
    ...
    //Get Targeted Player
    public static Player returnTargetPlayer(Player p, double distance, boolean allowThorughBlocks, Map<UUID, Long> cooldown, String prefix) {
        ...
        for(int i = 0; i<distance*2; i++) {
            ...
            double z = v.getZ() * t;
            loc.add(x,y,z);
            //loc.getWorld().spawnParticle(Particle.FLAME, loc, 20, 0, 0, 0, 0);
            if (!allowThorughBlocks){
                if (loc.getBlock().getType().name().contains("DOOR")){
                    Openable door = (Openable) loc.getBlock().getBlockData();
                    if (!door.isOpen()){
                        denyPassthough(p, cooldown, prefix);
                        return null;
                    }
                }
                else if (!loc.getBlock().isPassable()){ //Cant use through blocks and If block is full
                    denyPassthough(p, cooldown, prefix);
                    return null;
                }
            }
            ...
            Collection<Entity> nearby = loc.getWorld().getNearbyEntities(loc, 0.35, 0.35, 0.35);
        ...
        if (cooldown != null) cooldown.remove(p.getUniqueId());
        return null;
        //e = p.getWorld().rayTraceEntities(p.getEyeLocation(), v, 50).getHitEntity();
    }
    private static void denyPassthough(Player p, Map<UUID, Long> cooldown, String prefix){
        p.sendMessage(prefix+ChatColor.RED+"You cannot target a player through blocks with this ability!");
        p.playSound(p, Sound.BLOCK_NOTE_BLOCK_BIT, 1, 0.5f);
        if (cooldown != null) cooldown.remove(p.getUniqueId());
    }

//Previous methods w/o cooldown
