...
//Constructor
    ...
    public void endGame(int endingStateDelay, int arenaDeletionDelayTicks){
        ...
        Bukkit.getScheduler().runTaskLater(Core.getInstance(), () -> {
            ...
            new GameStateChangeEvent(arena, GameState.ENDING, pastGameState).callEvent();
        }, endingStateDelay);

        Bukkit.getScheduler().runTaskLater(Core.getInstance(), () -> {
            GameState pastGameState = arena.gameState;
            arena.gameState = GameState.DELETED;
            new GameStateChangeEvent(arena, GameState.DELETED, pastGameState).callEvent();
    ...
    public void forceDelete(){
        ...
        GameState pastGameState = gameState;
        gameState = GameState.DELETED;
        isInEndingProcess = true;
        isForceDeleted = true;
        new GameStateChangeEvent(this, GameState.DELETED, pastGameState).callEvent();
...
//Getters
    ...
    public boolean containsPlayer(@NotNull Player player){
        ...
        if (gameState == GameState.CONNECTING && startPlayers.contains(player)){
            return true;
        }
