...
//BossBars
    ...
    public static void removeFromBar(Player p, String barID){
        KeyedBossBar bar = Bukkit.getBossBar(new NamespacedKey(Core.getInstance(), barID));
        if (bar == null){
            return;
        }
        bar.removePlayer(p);
    ...
    public static KeyedBossBar getBossBar(String barID){
        return Bukkit.getBossBar(new NamespacedKey(Core.getInstance(), barID));
    }
    ...
    private static void trackHealth(KeyedBossBar bar, LivingEntity livingEntity){
        new BukkitRunnable(){
            public void run(){
                ...
                double progress = livingEntity.getHealth()/(livingEntity.getAttribute(Attribute.MAX_HEALTH).getValue());
                bar.setProgress(progress);
            }
        }.runTaskTimer(Core.getInstance(), 1, 3);
    }
    ...
    public static void removeBossBar(String barID){
        NamespacedKey key = new NamespacedKey(Core.getInstance(), barID);
    ...
    public static void removeAllBossBars(){
        ...
        for (Iterator<KeyedBossBar> it = Bukkit.getBossBars(); it.hasNext();) {
            ...
            if (key.getNamespace().equalsIgnoreCase(Core.getInstance().getName())){
                allBarIDs.add(key.value());
            }
        }

        for (String s : allBarIDs){
            NamespacedKey key = new NamespacedKey(Core.getInstance(), s);
            Bukkit.removeBossBar(key);
        }
    }
}
