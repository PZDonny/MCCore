...
//Player Ability Damage
    ...
    public static void setAbilityDamager(UUID victim, UUID attacker){
        if (victim != null & attacker != null){
            Bukkit
                    .getScheduler()
                    .runTaskLater(Core.getInstance(), () -> PlayerUtils.sendHPToPlayer(Bukkit.getPlayer(attacker), Bukkit.getPlayer(victim)), 2);
        }
...
//Fall Damage Resistance
            ...
            public void run(){
                if (player.isOnGround() && fallDamageResistant.contains(player.getUniqueId())){
                    new BukkitRunnable(){
                        @Override
                        public void run() {
                            removePlayerFallDamageResistance(player);
                        }
    ...
    public static void removePlayerFallDamageResistance(Player player){
        fallDamageResistant.remove(player.getUniqueId());
    }

    public static boolean isPlayerFallDamageResistant(Player player){
