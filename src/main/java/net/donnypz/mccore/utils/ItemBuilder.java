...
public class ItemBuilder implements Cloneable{
    ...
    public ItemBuilder setUseCooldown(float cooldownInSeconds, @Nullable String cooldownGroupKey){
        if (cooldownGroupKey == null){
            ...
            setUseCooldown(cooldownInSeconds, new NamespacedKey(Core.getInstance(), cooldownGroupKey));
        }
        return this;
    }
    ...
    public ItemBuilder setFood(@NotNull FoodProperties.Builder builder){
        ItemUtils.setFood(itemStack, builder);
        return this;
    }

    public ItemBuilder setFood(boolean alwaysEat, int nutrition, float saturation){
        setFood(FoodProperties
                .food()
    ...
    public ItemBuilder setConsumable(@NotNull Consumable.Builder builder){
        ItemUtils.setConsumable(itemStack, builder);
        return this;
    }

    public ItemBuilder setConsumable(ItemUseAnimation animation, float consumeSeconds, boolean showParticles, Key sound, Collection<ConsumeEffect> consumeEffects){
        ItemUtils.setConsumable(itemStack, animation, consumeSeconds, showParticles, sound, consumeEffects);
        return this;
    }

    public ItemBuilder setConsumable(ItemUseAnimation animation, float consumeSeconds, boolean showParticles, Key sound, ConsumeEffect consumeEffect){
        if (consumeEffect != null){
            return setConsumable(animation, consumeSeconds, showParticles, sound, List.of(consumeEffect));
        }
        else{
            return setConsumable(animation, consumeSeconds, showParticles, sound, (Collection<ConsumeEffect>) null);
        }
    }

    public ItemBuilder setConsumable(ItemUseAnimation animation, float consumeSeconds, boolean showParticles, @NotNull Sound sound, Collection<ConsumeEffect> consumeEffects){
        return setConsumable(animation, consumeSeconds, showParticles, Registry.SOUNDS.getKey(sound), consumeEffects);
    }

    public ItemBuilder setConsumable(ItemUseAnimation animation, float consumeSeconds, boolean showParticles, @NotNull Sound sound, ConsumeEffect consumeEffect){
        return setConsumable(animation, consumeSeconds, showParticles, Registry.SOUNDS.getKey(sound), consumeEffect);
    }

    public ItemBuilder setUseRemainder(@NotNull ItemStack item, ItemStack newItem){
        ItemUtils.setUseRemaineder(item, newItem);
        return this;
    }
        ...
        itemStack.editMeta(meta -> {
            ...
            //Attribute Required on an item to use Hide Attributes
                if (flag == ItemFlag.HIDE_ATTRIBUTES && !meta.hasItemFlag(ItemFlag.HIDE_ATTRIBUTES)){
                    meta.addAttributeModifier(Attribute.ENTITY_INTERACTION_RANGE, new AttributeModifier(new NamespacedKey(Core.getInstance(), "hide_attr_flag"), 0, AttributeModifier.Operation.ADD_NUMBER));
                }
